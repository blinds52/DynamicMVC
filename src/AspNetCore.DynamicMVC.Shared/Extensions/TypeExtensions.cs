...
namespace DynamicMVC.Shared.Extensions
{
    public static class TypeExtensions
    {
        ...
        {
            var types = type.GetTypeInfo().Assembly.GetTypes().ImplementsInterface<T>().ToList();
        ...
        {
            var types = type.GetTypeInfo().Assembly.GetTypes().ImplementsInterface(type).ToList();
            return types.Where(x => !(x == type) && !x.GetTypeInfo().IsAbstract).ToList();
        }
        ...
        {
            return type.GetTypeInfo().GetCustomAttributes().Any(x => x is T);
        }

        public static IEnumerable<Type> GetTypesWithAttribute<T>(this IEnumerable<Type> types) where T : Attribute
        {
            return types.GetPublicNonAbstractTypes().Where(t => t.GetTypeInfo().GetCustomAttributes().Any(x => x.GetType() == typeof(T)));
        }

        public static IEnumerable<Type> GetPublicNonAbstractTypes(this IEnumerable<Type> types)
        {
            return types.Where(t => t != null && t.GetTypeInfo().IsPublic && !t.GetTypeInfo().IsAbstract);
        }
    }
}
